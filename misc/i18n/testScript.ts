...
async function importTextmap(suffix: string){
  let output = {}
  await import(`./TextMap${suffix}.json`)
    .then((data) => {
      console.log('loaded data successfully')
      output = data
    })
    .catch((e) => console.log('error importing JSON: ', e))
  return output
}
  ...
  for (const locale of Locales) {
    ...
    for (const lightcone of lightconeConfig) {
      const Lightcone: Lightcone = {
        ...
        SkillName: cleanString(locale, textmap[lightconeRankConfig.filter((x) => x.SkillID == lightcone.SkillID).sort((a, b) => a.Level - b.Level).pop()!.SkillName.Hash]),
      }
...
function tbIdToNativeName(id, textmap: TextMap, pathmap: Path[], locale: string) {
