...
export function CharacterCardScoringStatUpgrades(props: { result: SimulationScore }) {
  ...
  const displayStats = calculateDisplayStats(result)
  ...
  return (
    <Flex vertical gap={1} align="center" style={{ paddingLeft: 6, paddingRight: 8, marginBottom: 0 }}>
      ...
      <Flex align='center' gap={5} justify='center' style={{ width: 230, marginTop: 5, marginBottom: 2 }}>
        <StatTag stat={Stats.SPD} value={'x8'}/>
        <StatTag stat={Stats.ATK_P} value={'x10'}/>
        <StatTag stat={Stats.CR} value={'x3'}/>
        <StatTag stat={Stats.CD} value={'x4'}/>
      </Flex>
      {rows}
    </Flex>
  )
...
function calculateDisplayStats(result: SimulationScore) {
  const simStats = result.originalSim.request.stats
  console.debug(result)
}

export function StatTag(props) {
  return (
    <Flex style={{ flex: 1 }}>
      <Tag
        bordered={false}
        style={{ paddingInline: '4px', marginInlineEnd: '0px', width: '100%' }}
        // style={{ paddingInlineEnd: '5px', paddingInlineStart: '3px', marginInlineEnd: '0px' }}
      >
        <Flex align='center' justify='center'>
          <img src={Assets.getStatIcon(props.stat)} style={{ width: iconSize, height: iconSize, marginRight: 5 }}/>
          {props.value}
        </Flex>
      </Tag>
    </Flex>
