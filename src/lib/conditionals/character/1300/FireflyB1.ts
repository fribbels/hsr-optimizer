...
export const FireflyB1Entities = createEnum('FireflyB1')
export const FireflyB1Abilities: AbilityKind[] = [
  AbilityKind.BASIC,
  AbilityKind.SKILL,
  AbilityKind.BREAK,
]
...
const conditionals = (e: Eidolon, withContent: boolean): CharacterConditionalsController => {
  const t = TsUtils.wrappedFixedT(withContent).get(null, 'conditionals', 'Characters.FireflyB1')
  ...
  return {
    ...
    entityDeclaration: () => Object.values(FireflyB1Entities),
    entityDefinition: (action: OptimizerAction, context: OptimizerContext) => ({
      [FireflyB1Entities.FireflyB1]: {
        primary: true,
        summon: false,
        memosprite: false,
      },
    ...
    actionDeclaration: () => [...FireflyB1Abilities],
...
if (${wgslTrue(r.superBreakDmg && r.enhancedStateActive)} && be >= 3.00) {
  ${buff.action(AKey.SUPER_BREAK_MODIFIER, 0.50).wgsl(action)}
    ...
    dynamicConditionals: [
      {
        id: 'FireflyB1ConversionConditional',
