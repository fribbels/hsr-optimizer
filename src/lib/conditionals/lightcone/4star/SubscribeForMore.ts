...
const conditionals = (s: SuperImpositionLevel, withContent: boolean): LightConeConditionalsController => {
  ...
  return {
    ...
    precomputeEffectsContainer: (x: ComputedStatsContainer, action: OptimizerAction, context: OptimizerContext) => {
      ...
      x.buff(StatKey.BOOST, sValues[s], x.damageType(DamageTag.BASIC | DamageTag.SKILL).source(SOURCE_LC))
      x.buff(StatKey.BOOST, (r.maxEnergyDmgBoost) ? sValues[s] : 0, x.damageType(DamageTag.BASIC | DamageTag.SKILL).source(SOURCE_LC))
    },
  }
}
