...
const conditionals = (s: SuperImpositionLevel, withContent: boolean): LightConeConditionalsController => {
  const t = wrappedFixedT(withContent).get(null, 'conditionals', 'Lightcones.IVentureForthToHunt')
  ...
  const content: ContentDefinition<typeof defaults> = {
    luminfluxUltStacks: {
      ...
      content: t('Content.luminfluxUltStacks.content', { DefIgnore: precisionRound(100 * sValuesDefShred[s]) }),
      min: 0,
      max: 2,
    },
  }
