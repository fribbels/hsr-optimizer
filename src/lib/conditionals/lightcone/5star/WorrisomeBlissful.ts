...
const conditionals = (s: SuperImpositionLevel, withContent: boolean): LightConeConditionalsController => {
  const t = wrappedFixedT(withContent).get(null, 'conditionals', 'Lightcones.WorrisomeBlissful')
  const { SOURCE_LC } = Source.lightCone(WorrisomeBlissful.id)
  ...
  const content: ContentDefinition<typeof defaults> = {
    targetTameStacks: {
      ...
      content: t('Content.targetTameStacks.content', { CritBuff: precisionRound(100 * sValuesCd[s]) }),
      min: 0,
      max: 2,
    },
  }
...
export const WorrisomeBlissful: LightConeConfig = {
  id: '23016',
  conditionals,
}
