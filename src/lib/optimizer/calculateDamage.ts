...
import { ComputedStatsArray, getElementalDamageType, getResPenType, Source } from 'lib/optimizer/computedStatsArray'
...
export function calculateDamage(x: ComputedStatsArray, action: OptimizerAction, context: OptimizerContext) {
  ...
  const baseResistance = context.enemyDamageResistance - x.$RES_PEN - context.combatBuffs.RES_PEN - getResPenType(x, context.elementalResPenType)
...
function calculateEhp(x: ComputedStatsArray, context: OptimizerContext) {
  ...
  let ehp = x.$HP / (1 - x.$DEF / (x.$DEF + 200 + 10 * context.enemyLevel))
...
function calculateShield(x: ComputedStatsArray, context: OptimizerContext) {
  ...
  x.SHIELD_VALUE.set(x.$SHIELD_VALUE * (1 + 0.20 * p4(sets.KnightOfPurityPalace)), Source.NONE)
}
...
function calculateAbilityDmg(
  ...
  if (abilityDmg) {
    const abilityCr = Math.min(1, x.$CR + abilityCrBoost)
    const abilityCd = x.$CD + abilityCdBoost
  ...
  if (abilityAdditionalDmg > 0) {
    const additionalDmgCr = abilityAdditionalCrOverride || Math.min(1, x.$CR)
    const additionalDmgCd = abilityAdditionalCdOverride || x.$CD
...
function calculateEhrMulti(
  ...
  const effectiveDotChance = Math.min(1, x.$DOT_CHANCE * (1 + x.$EHR) * (1 - enemyEffectRes + x.$EFFECT_RES_PEN))
