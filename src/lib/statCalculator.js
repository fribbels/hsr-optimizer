...
export const StatCalculator = {
  ...
  calculate(character) {
    ...
    let hero = {
      [Constants.Stats.HP]:  (base[Constants.Stats.HP]  + lc[Constants.Stats.HP])  * (1 + 0.12 * Math.min(1, ornamentSets[1] >> 1) + 0.12 * Math.min(1, relicSets[12] >> 1) + sum(relics, Constants.Stats.HP_P)  + trace[Constants.Stats.HP_P])  + sum(relics, Constants.Stats.HP),
      [Constants.Stats.ATK]: (base[Constants.Stats.ATK] + lc[Constants.Stats.ATK]) * (1 + 0.12 * Math.min(1, ornamentSets[0] >> 1) + 0.12 * Math.min(1, ornamentSets[10] >> 1) + 0.12 * Math.min(1, relicSets[1] >> 1) + 0.12 * Math.min(1, relicSets[15] >> 1) + sum(relics, Constants.Stats.ATK_P) + trace[Constants.Stats.ATK_P]) + sum(relics, Constants.Stats.ATK),
      [Constants.Stats.DEF]: (base[Constants.Stats.DEF] + lc[Constants.Stats.DEF]) * (1 + 0.15 * Math.min(1, ornamentSets[3] >> 1) + 0.15 * Math.min(1, relicSets[2] >> 1) + sum(relics, Constants.Stats.DEF_P) + trace[Constants.Stats.DEF_P]) + sum(relics, Constants.Stats.DEF),
    ...
    let dmg = (hero[Constants.Stats.ATK] + form.buffAtk + (form.buffAtkP * base[Constants.Stats.ATK])) * (1 + hero[Constants.Stats.CD] + form.buffCd) * cappedCrit * (1 + elementalDmg)
    let mcd = (hero[Constants.Stats.ATK] + form.buffAtk + (form.buffAtkP * base[Constants.Stats.ATK])) * (1 + hero[Constants.Stats.CD] + form.buffCd) * (1 + elementalDmg)
