...
export const StatCalculator = {
  ...
  calculateCharacterWithRelics(character, relics) {
    ...
    let hero = {
      ...
      [Constants.Stats.CR]: 0.04 * Math.min(1, relicSets[16] >> 2) + 0.08 * Math.min(1, ornamentSets[5] >> 1) + 0.08 * Math.min(1, ornamentSets[8] >> 1) + (base[Constants.Stats.CR] + lc[Constants.Stats.CR] + crSum + trace[Constants.Stats.CR]),
