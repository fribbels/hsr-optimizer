...
export const StatCalculator = {
  ...
  calculateCharacterWithRelics(character, relics) {
    ...
    const hero = {
      ...
      [Constants.Stats.RES]: 0.1 * Math.min(1, ornamentSets[16] >> 1) + 0.1 * Math.min(1, ornamentSets[9] >> 1) + (base[Constants.Stats.RES] + lc[Constants.Stats.RES] + sum(relics, Constants.Stats.RES) + trace[Constants.Stats.RES]),
