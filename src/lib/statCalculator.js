...
export const StatCalculator = {
  ...
  calculateCharacterWithRelics(character, relics) {
    ...
    const hero = {
      ...
      [Constants.Stats.CD]: 0.16 * Math.min(1, ornamentSets[17] >> 1) + 0.16 * Math.min(1, ornamentSets[4] >> 1) + (base[Constants.Stats.CD] + lc[Constants.Stats.CD] + cdSum + trace[Constants.Stats.CD]),
      ...
      [Constants.Stats.ERR]: 0.05 * Math.min(1, ornamentSets[16] >> 1) + 0.05 * Math.min(1, ornamentSets[7] >> 1) + 0.05 * Math.min(1, ornamentSets[11] >> 1) + (base[Constants.Stats.ERR] + lc[Constants.Stats.ERR] + sum(relics, Constants.Stats.ERR) + trace[Constants.Stats.ERR]),
