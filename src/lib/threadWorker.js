const calculateStats = function(e) {
  ...
  for (let row = 0; row < data.HEIGHT; row++) {
    for (let col = 0; col < data.WIDTH; col++) {
      ...
      hero[Stats.BE] =  0.16 * Math.min(1, ornamentSet6 >> 1) + 0.16 * Math.min(1, relicSet10 >> 1) + 0.16 * (relicSet10 >> 2) + (base[Stats.BE]  + lc[Stats.BE]  + sum(h, g, b, f, p, l, Stats.BE)  + trace[Stats.BE])
