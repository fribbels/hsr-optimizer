...
test('Switch relics between characters in Characters tab', async ({ page }) => {
  ...
  // Body
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15ATK %43\.2%HP71DEF %9\.1%SPD4Break Effect12\.3%Score15\.9 \(C\)$/ })).toHaveCount(1)
  // Boots
  ...
  // Orb
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15Lightning DMG38\.8%ATK59ATK %8\.2%DEF %5\.4%Effect HIT6\.9%Score15\.5 \(\?\)$/ })).toHaveCount(1)
  // Rope
  ...
  // Hands
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15ATK352HP %3\.4%SPD4CRIT DMG22\.6%Effect HIT3\.4%Score15\.0 \(C\)$/ })).toHaveCount(1)
  // Body
  ...
  // Orb
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15Ice DMG38\.8%DEF74CRIT Rate5\.5%CRIT DMG12\.3%Effect HIT4\.3%Score4\.1 \(\?\)$/ })).toHaveCount(1)
  // Rope
