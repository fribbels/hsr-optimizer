...
test('Switch relics between characters in Characters tab', async ({ page }) => {
  ...
  await expect(page.locator('body')).toContainText('Successfully switched relics with (Old) Kafka')
  ...
  // Hands
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15ATK352HP42ATK %15\.1%CRIT DMG11\.6%Break Effect5\.8%Score28\.7 \(A\+\)$/ }).first()).toBeVisible()
  // Body
  ...
  // Orb
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15Lightning DMG38\.8%ATK59ATK %8\.2%DEF %5\.4%Effect HIT6\.9%Score14\.7 \(\?\)$/ }).first()).toBeVisible()
  // Rope
  ...
  // Head
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15HP705CRIT Rate11\.0%CRIT DMG10\.3%Effect RES3\.4%Break Effect5\.1%Score0\.0 \(F\)$/ }).first()).toBeVisible()
  // Hands
  ...
  // Body
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15CRIT DMG64\.8%HP114ATK %7\.3%DEF %4\.3%CRIT Rate8\.1%Score13\.9 \(\?\)$/ }).first()).toBeVisible()
  // Boots
  ...
  // Orb
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15Ice DMG38\.8%DEF74CRIT Rate5\.5%CRIT DMG12\.3%Effect HIT4\.3%Score3\.6 \(\?\)$/ }).first()).toBeVisible()
  // Rope
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15ATK %43\.2%HP80CRIT Rate8\.4%CRIT DMG10\.3%Break Effect5\.8%Score5\.1 \(F\)$/ }).first()).toBeVisible()
})
