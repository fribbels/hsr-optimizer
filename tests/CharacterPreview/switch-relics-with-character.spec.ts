...
test('Switch relics between characters in Characters tab', async ({ page }) => {
  ...
  // Hands
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15ATK352HP %3\.4%SPD4CRIT DMG22\.6%Effect HIT3\.4%Score24\.3 \(B\+\)$/ })).toHaveCount(1)
  // Body
  ...
  // Orb
  await expect(page.locator('.ant-card').filter({ hasText: /^\+15Ice DMG38\.8%DEF74CRIT Rate5\.5%CRIT DMG12\.3%Effect HIT4\.3%Score14\.9 \(D\+\)$/ })).toHaveCount(1)
  // Rope
